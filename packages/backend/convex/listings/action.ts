...
import {
	LinkedInJobsActor,
	type LinkedInJobsInput,
} from "../driver/jobs/actors/linkedin_jobs";
import {
	IndeedJobsActor,
	type IndeedJobsInput,
} from "../driver/jobs/actors/indeed_jobs";
...
export const addNewJobsListingAction = internalAction({
	...
	handler: async (ctx, args) => {
		...
		const [linkedInJobSearchResults, indeedJobSearchResults] =
			await Promise.all([
		...
		const linkedInJobs: MinimalLinkedInJob[] = (
			linkedInJobSearchResults as any[]
		).flatMap((r) => {
			console.log("Processing LinkedIn result item:", {
				...
				length:
					"linkedInJobs" in r && Array.isArray(r.linkedInJobs)
						? r.linkedInJobs.length
						: 0,
				keys: Object.keys(r || {}),
			});
		...
		const indeedJobs: MinimalIndeedJob[] = (
			indeedJobSearchResults as any[]
		).flatMap((r) => {
