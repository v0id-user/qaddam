...
export const migrateJobs = internalMutation({
  ...
  handler: async (ctx, { jobListings }): Promise<GenericId<"jobListings">[]> => {
    ...
    for (const jobData of jobListings) {
      try {
        ...
        if (!jobData.title || !jobData.description || !jobData.company || !jobData.url) {
          console.warn(`Skipping job due to missing essential fields: ${jobData.sourceId || 'unknown'}`);
          skippedJobs++;
          continue;
        }
    ...
    console.log(`Successfully inserted ${insertedJobs.length} job listings, skipped ${skippedJobs} invalid jobs`);
    return insertedJobs;
  },
});
...
function extractEssentialJobData(rawJob: any): SimpleJobData | null {
  try {
    ...
    if (rawJob.salaryInfo && Array.isArray(rawJob.salaryInfo) && rawJob.salaryInfo.length > 0) {
...
export const migrateJobsAction = internalAction({
    handler: async (ctx): Promise<GenericId<"jobListings">[]> => {
      ...
      const files = await Promise.all(linkedInJobsStorageIds.map(async (id) => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          console.log(`Storage ${storageId}: Processed ${jobsProcessed} jobs, skipped ${jobsSkipped} invalid jobs`);
        } catch (error) {
          console.error(`Failed to parse job data from storage ID ${storageId}:`, error);
          skippedFiles++;
        }
      }

      console.log(`Found ${allJobListings.length} valid job listings from ${files.length - skippedFiles} files (${skippedFiles} files skipped)`);
      ...
      for (let i = 0; i < allJobListings.length; i += batchSize) {
        ...
        const batchIds: GenericId<"jobListings">[] = await ctx.runMutation(internal.scripts.saveJobs.migrateJobs, {
          jobListings: batch
        });
        insertedJobIds.push(...batchIds);
        
        console.log(`Batch ${Math.floor(i / batchSize) + 1}: Inserted ${batchIds.length}/${batch.length} jobs`);
      }

      console.log(`Migration complete! Inserted ${insertedJobIds.length} jobs total`);
      return insertedJobIds;
    }
});

// Can be called thro CLI:
// example: npx convex run --local --script saveJobs.runMigration
