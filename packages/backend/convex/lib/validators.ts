	...
	return (
		...
		(record.companyInfo === undefined ||
			(typeof record.companyInfo === "object" &&
				...
				((record.companyInfo as Record<string, unknown>).companyLogo === null ||
					typeof (record.companyInfo as Record<string, unknown>).companyLogo ===
						"string") &&
				((record.companyInfo as Record<string, unknown>).companyDescription ===
					null ||
					typeof (record.companyInfo as Record<string, unknown>)
						.companyDescription === "string") &&
				((record.companyInfo as Record<string, unknown>).indeedUrl ===
					undefined ||
					typeof (record.companyInfo as Record<string, unknown>).indeedUrl ===
						"string") &&
				((record.companyInfo as Record<string, unknown>).url === null ||
					typeof (record.companyInfo as Record<string, unknown>).url ===
						"string") &&
				((record.companyInfo as Record<string, unknown>).rating === null ||
					typeof (record.companyInfo as Record<string, unknown>).rating ===
						"number") &&
				((record.companyInfo as Record<string, unknown>).reviewCount === null ||
					typeof (record.companyInfo as Record<string, unknown>).reviewCount ===
						"number") &&
				((record.companyInfo as Record<string, unknown>).companySize === null ||
					typeof (record.companyInfo as Record<string, unknown>).companySize ===
						"object")))
	);
};
...
export const isCrawledJobs = (obj: unknown): obj is CrawledJobs => {
	...
	return (
		(record.source === "linkedIn" ||
			record.source === "linked-in" ||
			record.source === "indeed") &&
		Array.isArray(record.jobs) &&
		record.jobs.every((job: unknown) =>
			record.source === "linkedIn" || record.source === "linked-in"
				? isLinkedInJob(job)
				: isIndeedJob(job),
		)
	);
...
export const validateCrawledJobsArray = (data: unknown): CrawledJobs[] => {
	...
	for (let i = 0; i < data.length; i++) {
		...
		if (isCrawledJobs(item)) {
			...
			throw new Error(
				`Invalid crawled jobs data at index ${i}: ${itemPreview}${itemPreview.length >= 200 ? "..." : ""}`,
			);
		}
	}
...
export const validateCVProfile = (data: unknown): CVProfile => {
	...
	const profile: CVProfile = {
		skills: Array.isArray(record.skills)
			? record.skills.filter((s: unknown) => typeof s === "string")
			: [],
		experience_level: ["entry", "mid", "senior", "executive"].includes(
			record.experience_level as string,
		)
			? (record.experience_level as "entry" | "mid" | "senior" | "executive")
			: "entry",
		job_titles: Array.isArray(record.job_titles)
			? record.job_titles.filter((t: unknown) => typeof t === "string")
			: [],
		industries: Array.isArray(record.industries)
			? record.industries.filter((i: unknown) => typeof i === "string")
			: [],
		keywords: Array.isArray(record.keywords)
			? record.keywords.filter((k: unknown) => typeof k === "string")
			: [],
		education: typeof record.education === "string" ? record.education : "",
		years_of_experience:
			typeof record.years_of_experience === "number"
				? Math.max(0, record.years_of_experience)
				: 0,
		preferred_locations: Array.isArray(record.preferred_locations)
			? record.preferred_locations.filter((l: unknown) => typeof l === "string")
			: [],
	};
...
export const validateJobRanking = (data: unknown): JobRanking => {
	...
	const ranking: JobRanking = {
			...
			? record.ranked_jobs
					...
					.map((job: unknown) => {
						...
						return {
							...
							match_reasons: (jobRecord.match_reasons as unknown[]).filter(
								(r: unknown) => typeof r === "string",
							) as string[],
							concerns: (jobRecord.concerns as unknown[]).filter(
		...
		insights: {
			total_relevant:
				typeof (record.insights as Record<string, unknown>)?.total_relevant ===
				"number"
					? ((record.insights as Record<string, unknown>)
							.total_relevant as number)
					: 0,
			avg_match_score:
				typeof (record.insights as Record<string, unknown>)?.avg_match_score ===
				"number"
					? ((record.insights as Record<string, unknown>)
							.avg_match_score as number)
					: 0,
			top_skills_in_demand: Array.isArray(
				(record.insights as Record<string, unknown>)?.top_skills_in_demand,
			)
				? ((
						(record.insights as Record<string, unknown>)
							.top_skills_in_demand as unknown[]
					).filter((s: unknown) => typeof s === "string") as string[])
				: [],
			salary_insights:
				typeof (record.insights as Record<string, unknown>)?.salary_insights ===
				"string"
					? ((record.insights as Record<string, unknown>)
							.salary_insights as string)
					: "",
			market_observations:
				typeof (record.insights as Record<string, unknown>)
					?.market_observations === "string"
					? ((record.insights as Record<string, unknown>)
							.market_observations as string)
					: "",
		},
	};
	...
	const analysis: BatchJobAnalysis = {
		jobAnalyses: Array.isArray(record.jobAnalyses)
			? record.jobAnalyses
					...
					.map((job: unknown) => {
						const jobRecord = job as Record<string, unknown>;
						const experienceMatch = jobRecord.experienceMatch as Record<
							string,
							unknown
						>;
						const locationMatch = jobRecord.locationMatch as Record<
							string,
							unknown
						>;
						const benefits = jobRecord.benefits as unknown[];
						const requirements = jobRecord.requirements as unknown[];
						const dataExtraction = jobRecord.dataExtraction as Record<
							string,
							unknown
						>;
						...
						return {
							...
							experienceMatch: {
								match_level: [
									...
									"poor",
									"mismatch",
								].includes(experienceMatch?.match_level as string)
									? (experienceMatch.match_level as
											| "perfect"
											| "good"
											| "partial"
											| "poor"
									...
									: "poor",
								match_score:
									typeof experienceMatch?.match_score === "number"
										? Math.max(0, Math.min(1, experienceMatch.match_score))
										: 0,
								match_reasons: Array.isArray(experienceMatch?.match_reasons)
									? ((experienceMatch.match_reasons as unknown[]).filter(
											(r: unknown) => typeof r === "string",
										) as string[])
									: [],
								experience_gaps: Array.isArray(experienceMatch?.experience_gaps)
									? ((experienceMatch.experience_gaps as unknown[]).filter(
											(g: unknown) => typeof g === "string",
										) as string[])
									: [],
								recommendation:
									typeof experienceMatch?.recommendation === "string"
										? (experienceMatch.recommendation as string)
										: "",
							},
							locationMatch: {
								match_score:
									typeof locationMatch?.match_score === "number"
										? Math.max(0, Math.min(1, locationMatch.match_score))
										: 0,
								match_reasons: Array.isArray(locationMatch?.match_reasons)
									? ((locationMatch.match_reasons as unknown[]).filter(
											(r: unknown) => typeof r === "string",
										) as string[])
									: [],
								work_type_match:
									typeof locationMatch?.work_type_match === "string"
										? (locationMatch.work_type_match as string)
										: "unknown",
							},
							benefits: Array.isArray(benefits)
								? benefits
										.filter((benefit: unknown) => {
										...
										.map((benefit: unknown) => {
											...
											return {
												...
												details:
													typeof benefitRecord.details === "string"
														? benefitRecord.details
														: undefined,
											};
										})
								: [],
								...
								? requirements
										...
										.map((req: unknown) => {
											...
											return {
												...
												details:
													typeof reqRecord.details === "string"
														? reqRecord.details
														: undefined,
											};
										})
							...
							dataExtraction: {
								salary: {
									is_salary_mentioned:
										typeof (dataExtraction?.salary as Record<string, unknown>)
											?.is_salary_mentioned === "boolean"
											? ((dataExtraction.salary as Record<string, unknown>)
													.is_salary_mentioned as boolean)
											: false,
									min:
										typeof (dataExtraction?.salary as Record<string, unknown>)
											?.min === "number"
								...
								company: {
									is_company_mentioned:
										typeof (dataExtraction?.company as Record<string, unknown>)
											?.is_company_mentioned === "boolean"
											? ((dataExtraction.company as Record<string, unknown>)
													.is_company_mentioned as boolean)
											: false,
									name:
										typeof (dataExtraction?.company as Record<string, unknown>)
											?.name === "string"
								...
								job_type: {
									type:
										typeof (dataExtraction?.job_type as Record<string, unknown>)
											?.type === "string"
											? ((dataExtraction.job_type as Record<string, unknown>)
													.type as string)
											: "full_time",
								},
							},
