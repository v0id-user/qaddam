...
const normalize_linkedin_job = (raw: LinkedInJob): JobListingInsert | null => {
	try {
...
const normalize_indeed_job = (raw: IndeedJob): JobListingInsert | null => {
	try {
...
export const normalize_job = (
	raw: unknown,
	source: string,
): JobListingInsert | null => {
	try {
		if (source === "linked-in") {
...
export const add_new_jobs_listing = internalMutation({
	...
	handler: async (ctx, { jobSearchResults }): Promise<string[]> => {
		...
		for (const result of resultsArray) {
			...
			if (
				"linkedInJobs" in resultObj &&
				Array.isArray(resultObj.linkedInJobs)
			) {
				...
				"indeedJobs" in resultObj &&
				Array.isArray(resultObj.indeedJobs)
			) {
				// Process Indeed jobs
				...
				"source" in resultObj &&
				typeof resultObj.source === "string"
			) {
				...
				if (source === "linked-in" || source === "indeed") {
					...
					if (normalizedJob) {
						try {
							...
							console.log(`Failed to insert ${source} job`, { error });
							skippedJobs++;
						}
					} else {
						skippedJobs++;
				...
				const hasLinkedInFields =
					"id" in resultObj && "title" in resultObj && "link" in resultObj;
				const hasIndeedFields =
					"positionName" in resultObj &&
					"url" in resultObj &&
					"company" in resultObj;
				...
				if (hasLinkedInFields) {
					...
					if (normalizedJob) {
						try {
							...
							console.log("Failed to insert inferred LinkedIn job", { error });
							skippedJobs++;
						}
					} else {
						skippedJobs++;
					...
					if (normalizedJob) {
						try {
							...
							console.log("Failed to insert inferred Indeed job", { error });
							skippedJobs++;
						}
					} else {
						skippedJobs++;
					...
					console.log("Unable to determine job source", {
		...
		console.log("Job normalization completed", {
